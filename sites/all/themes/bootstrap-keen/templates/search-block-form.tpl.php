...
<script>
  Drupal.behaviors.ensureSearchFastAccess = {
    attach: function() {
      if (window.innerWidth < 992) {
        return;
      }
      if (!jQuery("#search-block-form .input-group-append").length) {
        let fast_access_search = "<?php echo str_replace(array("\r\n", "\r", "\n"), "", addslashes(theme('gofast_fastaccess_saved_searches'))) ?>";
        $("#search-block-form .input-group").append('<div class="input-group-append">' + fast_access_search + '</div>');
      }
    }
  }
</script>