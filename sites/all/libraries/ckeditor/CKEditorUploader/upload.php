...
// ALWAYS put the final slash (/)imageimport
$basePath = "sites/default/files/imageimport/";
...
$all_extension = array("png","jpg","tiff","gif","bmp",'tiff');
$all_mimetype = array("image/png","image/jpeg","image/tiff","image/gif","image/tiff","image/x-xbitmap");
...
if (isset($_FILES['upload'])) {
    ...
    $new_name_random = uniqid("GoFAST_") . uniqid();
    
    #  check if mimetype && extension are correct (prevent hacker)
    $mimetype = $_FILES['upload']['type'];
    $extension = pathinfo($name, PATHINFO_EXTENSION);
    
    if (in_array($extension, $all_extension) == FALSE && in_array($mimetype,$all_mimetype) == FALSE){
        echo ("You're not allowed to upload this kind of files");
        die(0);
    }
    // It doesn't care if the file already exists, it's simply overwritten.
    move_uploaded_file($_FILES["upload"]["tmp_name"], $baseUrl . $basePath . $new_name_random );

    // Build the url that should be used for this file   
    $url = $baseUrl . $new_name_random ;
...
$result = array(
  'filename' => $name,
  'uploaded' => 1,
  'url' => 'https://' . $_SERVER['SERVER_NAME'] . '/' . $basePath . $new_name_random , 
);
