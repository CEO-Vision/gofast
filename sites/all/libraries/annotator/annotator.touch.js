...
(function() {
  ...
  Annotator.Plugin.Touch = (function(_super) {
    ...
    /* patch ceo-vision*/
    if(parent.Drupal.settings.gofast.language === 'fr'){
      var textA = "Annoter";
      var TextB = "Commencer à annoter";
      var TextC = "J'ai fini";
    }else{
      var textA = "Annotate";
      var TextB = "Start Annotating";
      var TextC = "I'm Done";
    }
    ...
    Touch.prototype.template = "<div class=\"annotator-touch-widget annotator-touch-controls annotator-touch-hide\">\n" +
                                  "<div class=\"annotator-touch-widget-inner\">\n"+
                                      "<a class=\"annotator-button annotator-add annotator-focus\">"+ _t(textA) +"</a>\n"+
                                      "<a class=\"annotator-button annotator-touch-toggle\" data-state=\"off\">" + _t(TextB) +"</a>\n"+
                                   "</div>\n"+
                               "</div>";

    var templateDone = "<div class=\"annotator-touch-widget annotator-touch-controls\">\n" +
                          "<div class=\"annotator-touch-widget-inner\">\n"+
                            "<a class=\"annotator-button\" onclick=\"return screenModeClick('hide', event)\">" + _t(TextC) + "</a>\n"+
                          "</div>\n"+
                        "</div>";
    ...
    Touch.prototype.showControls = function() {
      ...
      $('#annotateTemplateDone').html(templateDone);
      if(!$(window.parent.document).find("#pdf_frame").hasClass("mode-gf-full")){
        $(window.parent.document).find("#pdf_frame").addClass("mode-gf-full");
        jQuery("#annotateTemplateDone").show();
        $("#gfPresentationMode").hide();
        if(parent.Drupal.settings.gofast.language === 'fr'){
          window.parent.window.Gofast.toast("Vous êtes maintenant en mode annotation (plein écran). Vous pouvez sortir de ce mode en utilisant le bouton \"J'ai fini\" en bas a droit.");
        }else{
          window.parent.window.Gofast.toast("You are now in annotation mode (full screen). You can exit this view using the buton \"I'm done\" at the right bottom corner.");
        }
        window.getSelection().removeAllRanges();
        setTimeout(show_again_annotation, 10);
      }
      return this;
    };
    ...
    Touch.prototype._setupControls = function() {
      jQuery("#annotateTemplateDone").hide();
    ...
    Touch.prototype._setupAnnotatorEvents = function() {
      ...
      this.annotator.editor.on("hide", function() {
        return _this.utils.nextTick(function() {
          ...
          //CEOV Workaround
          window.parent.Gofast.addLoading();

          setTimeout(function(){
            var currentValue = $("#scaleSelect").val();
            $("#scaleSelect").val(currentValue);
            $("#scaleSelect").focus();
            $("#scaleSelect").click();
            $("#scaleSelect").trigger("touch");
            $("#scaleSelect").trigger("tap");
            var event = new Event('change');
            $("#scaleSelect")[0].dispatchEvent(event);
            window.parent.Gofast.removeLoading();
            setTimeout(function(){
                window.parent.focus();
            }, 300);
          }, 1000);
      ...
      return this.annotator.viewer.on("hide", function() {
        return _this.utils.nextTick(function() {
          if (_this.highlighter) return _this.highlighter.enable().deselect(); $("#toolbarViewer").trigger("tap");
        });
      });
    };
    ...
    Touch.prototype._onSelection = function() {
      ...
      if (this.isAnnotating() && this.range && this._isValidSelection(this.range)) {
        ...
        var pathname = top.window.location.pathname; // Returns path
        var isNode = pathname.indexOf("node/"); // if path exist
        if (isNode > -1){
          return this.showControls();
        }
        return false;
      } else {
        if (jQuery("#annotateTemplateDone").hasClass("processed")){
          ...
          jQuery("#annotateTemplateDone").addClass("processed");
        }
      }
    };
  ...
  Annotator.Plugin.Touch.Editor = (function(_super) {
    ...
    Editor.prototype.templates = {
      quote: "<button class=\"annotator-quote-toggle\" onclick=\"return false;\" >" + _t("expand") + "</button>\n<span class=\"quote\"></span>"
    };
    ...
    Editor.prototype._setupQuoteField = function() {
      ...
      this.quote = jQuery(this.editor.addField({
        ...
        load: function(field, annotation) {
          _this.hideQuote();
          var spanQuoteText = "";
          annotation.highlights.forEach(element => {
            spanQuoteText += element.innerText + " ";
          });
          annotation.quote = spanQuoteText;
  ...
  Annotator.Plugin.Touch.utils = (function() {
    ...
    for (_i = 0, _len = vendors.length; _i < _len; _i++) {
      prefix = vendors[_i];
      if (!(!requestAnimationFrame)) continue;
  ...
  Annotator.Plugin.Touch.Viewer = (function(_super) {
    ...
    Viewer.prototype._onTap = function(event) {
      ...
      if (!isVisible) return target.addClass(this.viewer.classes.showControls);
    };
