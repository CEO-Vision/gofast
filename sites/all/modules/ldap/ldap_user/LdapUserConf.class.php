 ...
 */
 ...
 /**
  ...
  public function provisionLdapEntry($account, $ldap_user = NULL, $test_query = FALSE, $op = NULL) {
    /** @patch@ceo-vision */ 
    ...
    elseif (!isset($account->uid)) {
      watchdog('Ldap User', '$account is not a valid user object', array(), WATCHDOG_ERROR);
    }
    else {
      $user_entity = clone $account;
    }
    
    // Non-sense unless 1st condition above holds (moved to 1st condition block).
    //list($account, $user_entity) = ldap_user_load_user_acct_and_entity($account->name);
    ...
    elseif ($test_query) {
      ...
      if ($ldap_entry_created) {
        ...
        if (!$ldap_user_prov_entry_exists) {
          ...
          // Prevent call to user_load()/user_save() while saving the entity.
          if ($op === 'insert') {
            // Called within a hook_user_insert() implementation.
            $account->ldap_user_prov_entries = $user_entity->ldap_user_prov_entries;
            field_attach_update('user', $account);
          }
          else {
            // Check if user_load() could be avoided.
      ...
      elseif (is_array($proposed_ldap_entry) && isset($proposed_ldap_entry['dn'])) {
        ...
        if ($test_query) {
         ...
         //  //debug('modifyLdapEntry,dn=' . $proposed_ldap_entry['dn']);  //debug($attributes);
          ...
          //patch sjeandroz@ceo-vision.com
          $attributes["userpassword"] = htmlspecialchars_decode($attributes["userpassword"]);
           
          //patch sjeandroz@ceo-vision.com
          // if sasl is activated on this profil, don't sync password
          $is_sasl_activated = $account->sasl_auth[LANGUAGE_NONE][0]['value'];
          if($is_sasl_activated == 1){
              unset($attributes["userpassword"]);         
          }
          if(isset($attributes["userpassword"]) && $attributes["userpassword"] == ""){
              unset($attributes["userpassword"]);
          }       
  ...
  public function deleteProvisionedLdapEntries($account) {
    ...
    /** @patch@ceovision - This field is independant from the account language! */
