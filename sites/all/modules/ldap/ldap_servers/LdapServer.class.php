...
class LdapServer {
  ...
  function bind($userdn = NULL, $pass = NULL, $anon_bind = FALSE) {
    ...
    if ($anon_bind === TRUE) {
      ...
      //technical bind with manager account
      if(strpos($userdn, "cn=manager") !== false){         
        ...
        $bind_return = ldap_bind_ext($this->connection, $userdn, $pass, [['oid' => LDAP_CONTROL_PASSWORDPOLICYREQUEST],['oid' => LDAP_CONTROL_PASSWORDPOLICYRESPONSE]]);
         if ($bind_return && ldap_parse_result($this->connection, $bind_return, $errcode, $matcheddn, $errmsg, $referrals, $ctrls)) {            
             if($errcode === 0){
                 ...
                 gofast_ldap_manage_ppolicy_return_on_failed_login($userdn,$ctrls,$errcode,$errmsg,$this); 
                 return $errcode;
             }                                  
         }else{
              return LDAP_CONNECT_ERROR;
 ...
 */
  ...
  function modifyLdapEntry($dn, $attributes = array(), $old_attributes = FALSE) {
    ...
    //patch ceo-vision.com
     global $bypass_ldap_password_sync;
     if($bypass_ldap_password_sync == true){
         watchdog("debug ldap bypass",$dn);
    ...
    foreach ($attributes as $key => $cur_val) {
      ...
      // Encode the password according to the server settings.
      // Fail if the encoding failed.
      if ($key === "userpassword" && strpos($cur_val, "{SASL}") === FALSE) {
        //$encoded_password = ldap_servers_encode_text($cur_val, "md5");         
        $encoded_password = ldap_servers_encode_text(str_replace("&#039;", "'", $cur_val), "bcrypt");
        if($encoded_password !== FALSE) {
          $attributes[$key] = $encoded_password;
        }
        else {
          watchdog('ldap_servers', 'Could not encode password.  Aborting entry change.', array(), WATCHDOG_ERROR);
          return FALSE;
        }
      }

      if ($cur_val == '' && $old_value != '') { // remove empty attributes
  ...
  /**
   * Perform an LDAP delete with recursivity option.
   *
   * @contrib elavault@ceovision : added recursive option
   * @see function _rdelete()
   * 
   * If $recursive is TRUE, performs a recursive delete : in this case, if the
   * DN specified isn't a leaf, its children, and all their children are deleted
   * down the tree. No verification is done: if you add this switch, ldapdelete
  ...
  public function delete($dn, $recursive = FALSE) {
    ...
    if ($recursive === TRUE) {
      return $this->_rdelete($dn);
    }

    if (!$result = @ldap_delete($this->connection, $dn)) {
  ...
  /** 
   * @ceovision
   * Performs a recursive LDAP delete on entries under (dn) subtree, and on the
   * entry root as well. Use with care.
   *
   * @param string $dn
   * @return boolean result per ldap_delete
   */
  public function _rdelete($dn) {
    // Performs single level search (LDAP_SCOPE_ONELEVEL) to list child entries.
    $ls = @ldap_list($this->connection, $dn, 'ObjectClass=*');
    $children = @ldap_get_entries($this->connection, $ls);

    // Recursive deletion of all child entries
    for ($i = 0; $i < $children['count']; $i++) {
      $this->_rdelete($children[$i]['dn']);
    }
    
    // Delete the root entry (dn)
    if (!$result = @ldap_delete($this->connection, $dn)) {
      $error = "LDAP Server delete(%dn) in LdapServer::delete() Error Server ID = %sid, LDAP Err No: %ldap_errno LDAP Err Message: %ldap_err2str ";
      $tokens = array('%dn' => $dn, '%sid' => $this->sid, '%ldap_errno' => ldap_errno($this->connection), '%ldap_err2str' => ldap_err2str(ldap_errno($this->connection)));
      watchdog('ldap_server', $error, $tokens, WATCHDOG_ERROR);
  ...
  function ldapQuery($scope, $params) {
    ...
    switch ($scope) {
      ...
      case LDAP_SCOPE_BASE:
        ...
        elseif ($this->hasError() && $this->errorMsg('ldap') !== 'No such object') {
  ...
  */
   ...
   * @param boolean $recusrsive
   */
  public function groupRemoveGroup($group_dn, $only_if_group_empty = TRUE, $recursive = FALSE) {
    ...
    return $this->delete($group_dn, $recursive);
