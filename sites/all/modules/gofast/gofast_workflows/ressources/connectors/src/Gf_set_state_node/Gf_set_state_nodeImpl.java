...
*/
public class Gf_set_state_nodeImpl extends AbstractGf_set_state_nodeImpl {
	String token;
	@Override
	protected void executeBusinessLogic() throws ConnectorException{
		...
		//first, get all terms from vocabulary "state"
		String url = "http://localhost/api/taxonomy/terms?vocabulary_name=state";
		...
		try {
			...
			if(((LazyMap) json_result).containsKey(getEtat())) {
				Object term = ((LazyMap) json_result).get(getEtat());
				Object tid = ((LazyMap) term).get("tid");
				
				//now we have tid for wanted state, we can try to update GF node
				...
				String json = "{\"field_state\": "+tid+", \"nid\" : \""+getNid()+"\"}";
				...
				setOutputParameter("output","{\"message\":\"Unable to find the state\"}");
			}
