...
var menuIntervalId;
  ...
  Drupal.behaviors.gofastBlocks = {
    attach: function (context) {
      ...
      function unpinItems() {
        var pinedItems = $('.gofast-block').filter(function () {
...
//          }, 50000); //15 seconds
      ...
      $('.gofast-block', context).once('', function () {
        var item = $(this),
                block = item.children('.gofast-block-outer');
...
//}
  ...
  Drupal.behaviors.gofastBlockCollapseFilters = {
    attach: function (context, settings) {
      ...
      $('.gofast-block .view-filters:not(.filters-processed)').addClass('filters-processed').each(function () {
        var elem = $(this);
...
//    }
  ...
  //get main menu elements asynchronously
    ...
    attach: function (context, settings) {
      ...
      $('.gofast_menu_async:not(.processed)', context).addClass('processed').each(function () {
        var elem = $(this);
        var gid = $(this).attr("gid");
        var elemId = elem.attr("id");
        elem.prev().mouseenter(function () {
          //    menuIntervalId = setInterval(function(){Gofast.block.loadMenuAsync(gid, elemId)}, 10);
          Gofast.block.loadMenuAsync(gid, elemId);

        });
        elem.prev().mouseout(function () {
...
Gofast.Display_FavoritesFolders = function(event){
    $('#contentFavoritesTab').attr('class','');
    $('#foldersFavoritesTab').attr('class','active');
    if($('.block-bookmarks .gofast-block-inner .loader-blog').length == 0){
        if($('.block-bookmarks .gofast-block-inner section .view-gofast-flag-bookmarks').length !== 0){
            $('.block-bookmarks .gofast-block-inner section .view-gofast-flag-bookmarks').remove();
            $('.block-bookmarks .gofast-block-inner section').append('<div class="loader-blog"></div>');
        }else{
            $('.block-bookmarks .gofast-block-inner .dashboard-folders-placeholder').remove();
            $('.block-bookmarks .gofast-block-inner').append('<div class="loader-blog"></div>');
        }
        $.post(location.origin + "/gofast/bookmarks/favorites_folders").done(function(data){
            $('#path_pager_bookmarks_folders').remove();
            $('.block-bookmarks .gofast-block-inner .loader-blog').remove();
...
Gofast.Display_ContentFolders = function(event){
    $('#foldersFavoritesTab').attr('class','');
    $('#contentFavoritesTab').attr('class','active');
    if($('.block-bookmarks .gofast-block-inner .loader-blog').length == 0){
        if($('.block-bookmarks .gofast-block-inner section .view-gofast-flag-bookmarks').length !== 0){
            $('.block-bookmarks .gofast-block-inner section .view-gofast-flag-bookmarks').remove();
            $('.block-bookmarks .gofast-block-inner').append('<div class="loader-blog"></div>');
...
Drupal.behaviors.delayButton = {
    ...
    container.parents('.gofast-block').addClass('gofast-block-processed');
  };
