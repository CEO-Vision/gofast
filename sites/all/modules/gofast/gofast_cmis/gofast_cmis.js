      ...
      $.ajax({
        ...
        if (result.isEditable != 1) {
          Gofast.toast(Drupal.t('This document is being opened by OnlyOffice', {}, {'context' : 'gofast:gofast_cmis'}), 'warning');
            ...
            var extention = href.substring(href.lastIndexOf('.')+1, href.length) || href;
           ...
           if(documents_settings.ticket && href.length < documents_settings.ticket_path_length){
            ...
            if(Drupal.settings.gofast_cmis.documents_form_defaults.edit_libreoffice_documents == true && libreoffice_extensions.includes(extention)){
        ...
        onSuccess: function() {
            ...
            if (version === false) {
              ...
              Gofast.processAjax('/node/add/alfresco-item?dragdrop=1', true);
                ...
                $("#link_upload_new_version_modal").click();
            }
        }
      });
...
Drupal.behaviors.loadTemplate = {
  attach: function (context) {
   ...
   $.post( "/ztree/templates/async", { arr_values_combine: arr_values_combine, ztree_options: ztree_options, paths_options: paths_options })
      .done(function( ztree ) {
        ...
        $('#edit-group-template-file-body').append("<div id='loader_temp' class='loader-templates'></div>");
        ...
        var waitForztree = function (callback, count) {
          if ($('#edit-group-template-file-body').contents().has('#ztree_component_content_templates').length > 0 && $('#ztree_component_content_templates').is(':empty') == false) {
            callback();
          } else {
            setTimeout(function () {
              if (!count) {
              ...
              if (count < 500) { // Max CAllback 500 time
                waitForztree(callback, count);
              } else { return; }
            }, 250);
          }
        };

        waitForztree(function () {
          $('#loader_temp').remove();
        });
    ...
    if(templates_folder_params.length > 0){
      ...
      .done(function( ztree ) {
        ...
        if ($('#modalContent #gofast-ajax-file-browser-add-template-folder-form').length){
          modalContentResize(false,true);
        }
      });
    }
  }
  ...
  function gofastMissingProtocol(){
    //OS Detection
    var extension="msi";
    if (navigator.appVersion.indexOf("Win")!=-1) extension="msi";
    if (navigator.appVersion.indexOf("Mac")!=-1) extension="pkg";
    if (navigator.appVersion.indexOf("X11")!=-1) extension="deb";
    if (navigator.appVersion.indexOf("Linux")!=-1) extension="deb";
    var title = Drupal.t("Open a non Office document", {}, {context : 'gofast'});
    var title = Drupal.t("Open a non Office document", {}, {context : 'gofast'});
    var html = "<span style='display:inline-block;padding-top:10px;'>" + '<strong>' + Drupal.t("Step 1: Please download and install this software to open this document from your PC.", {}, {context: 'gofast'}) + '</strong>' + '</span><a onClick="window.open(\'' + location.origin + "/sites/all/libraries/ajax_file_browser/Plugins/ITHitEditDocumentOpener."+extension+"'" + ', \'_blank\')"><button type="button" class="btn btn-info" style="float:right;"><i class="fa fa-download" aria-hidden="true"></i> ITHitEditDocumentOpener.'+extension+'</button></a>';
    ...
    html += "<ul>";
    html += "<li>";
    html += "<span style='display:inline-block;padding-top:10px;'>"+'<a href="https://chrome.google.com/webstore/detail/it-hit-edit-doc-opener-5/nakgflbblpkdafokdokmjdfglijajhlp" target="_blank">' + '<img src="/sites/default/files/logos_web/google_chrome.png" height="3%" width="3%">',{}, {context: 'gofast-ithit-help'} + '</a>';
    html += Drupal.t(' Link for Chrome Extension',{}, {context: 'gofast-ithit-help'}) + '</strong> </a>';
    html += "</li>";
    html += "<li>";
    html += "<span style='display:inline-block;padding-top:10px;'>"+'<a href="https://addons.mozilla.org/fr/firefox/addon/it-hit-edit-doc-opener-5/" target="_blank">' + '<img src="/sites/default/files/logos_web/firefox.png" height="3%" width="3%">',{}, {context: 'gofast-ithit-help'} + '</a>';
    html += Drupal.t(' Link for FireFox Extension',{}, {context: 'gofast-ithit-help'}) + '</strong> </a>';
    html += "</li>";
    html += "<li>";
    html += "<span style='display:inline-block;padding-top:10px;'>"+'<a href="https://microsoftedge.microsoft.com/addons/detail/it-hit-edit-doc-opener-5/mdfaonmaoigngflemfmkboffllkopopm?hl=fr" target="_blank">' + '<img src="/sites/default/files/logos_web/edge.png" height="3%" width="3%">',{}, {context: 'gofast-ithit-help' }  + '</a>';
    html += Drupal.t(' Link for Edge Extension',{}, {context: 'gofast-ithit-help'}) + '</strong> </a>';
    html += "</li>";
    html += "</ul>";
    html += Drupal.t('For help: consult the ',{}, {context: 'gofast-ithit-help'}) + '<a href="https://gofast-docs.readthedocs.io/fr/latest/docs-gofast-users/doc-gofast-guide-utilisateurs.html" target="_blank"><strong>' + Drupal.t('documentation',{}, {context: 'gofast-ithit-help'}) + '</strong> </a>' + Drupal.t('or our ',{}, {context: 'gofast-ithit-help'}) + '<a href="https://community.ceo-vision.com/" target="_blank"><strong>' + Drupal.t('forum',{}, {context: 'gofast-ithit-help'}) + '</strong> </a>';
    html += "</span>";
