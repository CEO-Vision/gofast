<?php 
    $has_children = count((array) $book_element->children) > 0;
?>
<?php if ($depth == 0) : ?>
<table class="book_element-explorer-element-table w-100" id="<?= random_int(0, 999999) ?>">
<?php endif; ?>
    <!-- BOOK -->
    <tr data-nid="<?= $book_element->nid ?>" data-depth="<?= $depth ?>" id="<?= $book_element->id ?>" data-name="<?= $book_element->name ?>" data-pid="<?= $book_element->pid ?>" class="book-explorer-element book-explorer-element-<?php echo $depth == 0 ? 'visible' : 'collapsed'; ?>">
        ...
        <td class="book-file-explorer-element-name book-explorer-element-name w-100 d-flex align-items-center justify-content-between" title="<?= $book_element->description ?>" data-placement="top" data-toggle="tooltip" data-trigger="hover">
            <a class="wiki-tree-widget-item-name item-name" href="<?= $book_element->link_path ?>">
            ...
            <?php if (isset($book_element->canUpdate) && $book_element->canUpdate && (!isset($widget))) : ?>
                <div class="d-flex align-items-center mr-2 <?= ($book_element->edit_link) ? 'mx-1' : 'mx-2' ?>">
                    ...
                    <?php if($book_element->edit_link): ?>
                        <a href="<?= $wiki_edit_url ?>"style="width: 35%;" class="ctools-use-modal"><i class="fas fa-pen text-muted cursor-pointer"></i></a>
                    <?php endif ?>
                </div>
            <?php endif; ?>
        </td>
    ...
    if (count((array) $book_element->children) > 0) {
        $below_depth = $depth + 1;
        echo gofast_book_render_books_elements($book_element->children, $below_depth);
    }
    ?>
<?php if ($depth == 0) : ?>
</table>
