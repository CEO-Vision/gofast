...
(function ($) {
  ...
  Drupal.ckeditorInit = function (textarea_id) {
    ...
    if (CKEDITOR.loadFullCore) {
      CKEDITOR.on('loaded', function () {
        textarea_settings = Drupal.ckeditorLoadPlugins(textarea_id, textarea_settings);
   ...
   */
   Drupal.ckeditorLoadPlugins = function(textarea_id, textarea_settings) {
    ...
    // TEMPORARY : Tags autocomplete on broke ckeditor on safari
    if (navigator.userAgent.indexOf('Safari') != -1 && navigator.userAgent.indexOf('Chrome') == -1) {
    }else{
      //Manage autotag custom plugin to tag users if the textarea is a comment/remove it if not
      textarea_settings["extraPlugins"] += (textarea_id.includes("edit-comment-body-und-0-value") && !textarea_settings["extraPlugins"].includes("gofast_autotag")) ?  ",gofast_autotag" : "";
      if(!textarea_id.includes("edit-comment-body-und-0-value") && textarea_settings["extraPlugins"].includes("gofast_autotag")) {
          textarea_settings["extraPlugins"] = textarea_settings["extraPlugins"].replace(',gofast_autotag','');
      }
    }
